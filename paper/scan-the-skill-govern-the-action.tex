...
\bottomrule
\end{tabular}
\caption{Representative results; five of the nine shapes we tried defeat the
resolver. This is an illustration, not a systematic adversarial evaluation: we wrote
the nine shapes ourselves and a determined adversary would look harder.}
\label{tab:evade}
\end{table}
...
Over a spread of benign, ambiguous, and risky command shapes, resolution costs
\textbf{1.03\,$\mu$s} mean on one core of an Apple M-series laptop, 3.27\,$\mu$s on
the slowest shape (\texttt{curl \ldots | bash}), or about 971{,}000 classifications
per second. End to end the current CLI hook takes 67.6\,ms median and 81.1\,ms at
p95, dominated by process spawn and local IPC rather than by classification: an
engineering cost, not an architectural one.
...
leave it implied, quoting ClawScan's own summaries.
\texttt{neuralshift1/gmail-last5} ``hard-codes a specific Gmail account and lacks a
clear user confirmation step'' and has no executable block at all.
\texttt{arubiku/mia-twitter-stealth} is ``designed to automate Twitter/X account
actions while hiding from bot detection'' through individually unremarkable tool
calls. \texttt{douglascorrea/obsidian-remote} ``gives an agent very powerful access
to private notes and app internals without enough safety boundaries,'' a
scope-versus-purpose mismatch in which no single action is disproportionate. A
hardcoded recipient, a persuaded behaviour, an over-broad scope: three shapes of harm
that SkillSpector's Excessive Agency analysis exists to catch and that a function
reading only a command string cannot see. A runtime action gate is not a superset of
a semantic scanner and must not be deployed as one.
...
Every measurement here can be re-derived from public artifacts. The corpus is
OpenClaw's own MIT-licensed dataset at
\url{https://huggingface.co/datasets/OpenClaw/clawhub-security-signals}; the
scanner-agreement figures in \S\ref{sec:layers} are two SQL queries in its own
