...
Second, we still do not report a false-positive rate for the live gate: 148 of the
170 commands were held rather than blocked, and we did not adjudicate how many of
those an operator would have waved through. That number decides whether a control
survives contact with an operator, it is the failure mode \S\ref{sec:graduation} is
written to address, and it remains unmeasured. Third, the run is one agent, one
