...
The numbers reported in \S\ref{sec:divergence} and \S\ref{sec:runtime} came from
scripts we did not preserve, and their per-command outputs were not retained, so those
two results rest on our record of the run rather than on anything a reader can check.
We have since published \texttt{research/live\_agent\_study.py}, the harness those
studies should have been run from: it drives a model against a real
\texttt{SKILL.md}, classifies every command it issues through the released gateway,
and writes one line per command so the raw record survives. It executes nothing, for
reasons its documentation gives.

We want to be exact about what that does and does not fix. Running it produces a
\emph{fresh} study, not a reproduction of the figures below: a different model, a
different day, and a harness that reports rather than executes will not return 93
commands and 3 verbatim. So \S\ref{sec:divergence} and \S\ref{sec:runtime} remain
demonstrations we are reporting, and what is now reproducible is the method rather
than the measurement. \S\ref{sec:permission} and \S\ref{sec:threat} are reproducible
end to end.
