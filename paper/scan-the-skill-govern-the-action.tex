...
exact test puts the two runs genuinely apart ($p = 1.6 \times 10^{-5}$), and the
cause is not the turn budget, which is unchanged at four. It is that the earlier
setup did not give the agent working output, so it spent its turns establishing
whether its own tooling worked and never got as far as the install. With a container
behind it the agent completes the install step, and reaches the forbidden action
roughly six times as often.
...
That cuts toward understating reach, not overstating it, but it is a difference.
